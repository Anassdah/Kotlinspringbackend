package anass.springboot

import anass.springboot.dao.CategoryRepository
import anass.springboot.dao.ProductRepository
import anass.springboot.data.Category
import anass.springboot.data.Product
import anass.springboot.entities.ProductProjection
...
class SpringbootApplication : CommandLineRunner {
    @Autowired
    ...
    override fun run(vararg args: String?)  {
        ...
        categoryRepository?.findAll()?.forEach { category : Category? ->
            ...
            for(i in 1..10){
                val p = Product(RandomString.make(10),
                    100 + Random.nextInt(10000).toDouble(), Random.nextBoolean(),
                    Random.nextBoolean(),Random.nextBoolean(), category,"unknown.png")
